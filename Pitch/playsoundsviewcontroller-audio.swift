...
extension PlaySoundViewController: AVAudioPlayerDelegate {
    ...
    func connectAudioNodes(_ nodes: AVAudioNode...) {
        if let audioFormat = audioFile?.processingFormat{
            for x in 0..<nodes.count-1 {
                audioEngine.connect(nodes[x], to: nodes[x+1], format: audioFormat)
            }
        }else{
             showAlert(Alerts.AudioFileError, message: "could not file the audio file!")
        }
    ...
    func setPlayButtonsEnabled(_ enabled: Bool) {
        ...
        Rabit.isEnabled = enabled
        Low.isEnabled = enabled
        Echo.isEnabled = enabled
        Revert.isEnabled = enabled
    }
